...
&ensp;&ensp;&ensp;&ensp;所以，虽然自由态非常好用，但往往在实际场景中可能会面临复杂情况，如何选择同步模式则根据您自己的实际需求而定，从我在前三个Zero项目中的经历看起来，**同步模式**一般很少用，大部分场景中都使用了**异步模式**，若是异步模式则Envelop就只需考虑**入参**一种情况，返回值会直接被`io.vertx.core.Future`接管。
