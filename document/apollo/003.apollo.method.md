...
```
　　上边代码中实现了Http方法中的GET方法，当您使用工具postman测试的时候，就可以得到对应的响应结果信息，并且在启动zero容器时可以看到如下输出：
...
　　开发了上述接口过后，可以使用`Postman`对每个接口进行单独测试，分别得到对应的注释中的响应结果，注意此处对于`PUT/DELETE`使用了`@PathParam`，该注解也属于JSR311，唯一不同的是：`@BodyParam`**并不是JSR311中的注解，它是zero框架中的扩展注解，除开这些注释外zero还扩展了更多JSR311中的注解，形成统一规范的“框架协议”**——通常Body参数只在`POST`和`PUT`方法中使用，而不在`GET/DELETE`中使用。
...
>  zero中对于第三方开源项目的扩展，全部写在第三方的原始包中，这样可能会对读者产生误解，但这种方式更容易说明框架内部的某些第三方插件的源头，如扩展的JSR311在`javax.ws.rs`包中，扩展的Jackson在`com.fasterxml.jackson.databind`包中。
...
　　本章我们主要学习 zero 中如何处理RESTful常用的四种基本请求：`@GET, @POST, @DELETE, @GET`，这是很多开发人员的常用需求，并且简单谈到了 RESTful 的 Web服务的基本概念。

>  四叶葎是四叶草的另外一种别称，它表示“幸运”的语义，而四片叶子，影射到四种 Http 方法中，这四种方法也是 RESTful 开发中常用的方法。