...
&ensp;&ensp;&ensp;&ensp;本章节会使用一个特殊的Zero注解：`io.vertx.up.annotations.Codex`，该注解表示当前结构会启用Zero中的扩展验证功能。启用了`@Codex`注解过后，需要在项目的资源目录中提供验证配置文件：
...
&ensp;&ensp;&ensp;&ensp;在对应目录中创建示例中所需的验证规则文件：
