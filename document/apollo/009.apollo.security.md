...
&ensp;&ensp;&ensp;&ensp;到这里，Zero的安全部分的开发就告一段落，虽然部分内容和安全没有直接关系，但这些东西都是实战过程中有用的部分，可提供给您参考。若您使用了`zero-rbac`扩展模块，无疑可以直接忽略掉系统的认证、授权并拥有它所有的功能，包括在微服务环境下提供的认证授权服务器的功能。
