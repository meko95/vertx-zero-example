...
&ensp;&ensp;&ensp;&ensp;上述结构中sheet属性就是当前ExTable所属的Excel的sheet名称，而`name`和`description`对应数据模板的`{TABLE}`行的内容，ExConnect则直接对应到`vertx-excel.yml`的mapping节点的配置，一张表只拥有一个ExConnect，`fields`和`values`就不解释了。
...
&ensp;&ensp;&ensp;&ensp;这些配置中，Zero会去读取`src/main/resources`为根目录（生产环境就是当前目录）之下的对应数据文件，如果无法读取则为null。
