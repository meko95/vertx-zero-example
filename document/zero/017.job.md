...
&ensp;&ensp;&ensp;&ensp;上述结构中，有部分参数来自于Web请求（如`Envelop、Session、User、MultiMap`），您可能会困惑——后台任务由服务端主动发起，为何会收到来自前端的请求信息呢？
...
&ensp;&ensp;&ensp;&ensp;在您的环境中设计如下几个接口：

> 本章的任务管理器接口不以`/api`打头，即不过安全墙，实际项目中如您使用zero-jet的**动态任务**管理时，这些接口都是需要和权限管理挂钩的。

|HTTP方法|URI方法|含义|
|---|:---|:---|
|PUT|/hi/start/{code}|启动任务接口|
|PUT|/hi/stop/{code}|停止任务接口|
|GET|/hi/job/by-code/{code}|读取某个任务详细信息|
|GET|/hi/job/status/{code}|查看任务的运行状态|
