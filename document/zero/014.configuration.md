...
> 接下来按功能来看看各部分的详细配置，走走这张地图，详细配置图可参考：
...
该文件中合并表格中的`vertx-router.yml`和`vertx-deployment.yml`。
...
该文件中合并表格中的`vertx-init.yml`和`vertx-extension.yml`。
...
该文件中合并表格中的`vertx-shared.yml, vertx-feign.yml, vertx-iqiy.yml, vertx-sms.yml`。
...
该文件中合并表格中的`vertx-rpc.yml、vertx-etcd.yml`。
...
&ensp;&ensp;&ensp;&ensp;本章节带您领略了整个Zero框架中的所有配置，详细的配置图可参考本文最开始的配置地图专用链接下载该图（2.8M），后续更新后会有新版本的配置完整地图。这些配置中某些配置部分是还没讲解的章节，而大部分都是已经讲解过的章节，有关配置的细节只能读者自己在学习过程去体会。
