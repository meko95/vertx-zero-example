...
&ensp;&ensp;&ensp;&ensp;那么到这里，整个Zero中目前牵涉的三个和Vert.x的集成就告一段落，根据本章节的内容，读者可以很快和MySQL、Mongo、Redis集成，并去详细体会Zero中的`Infix`插件的详细内容，后续版本中，我会根据实际项目需求将部分客户端内容标准化，当前版本则提供给读者自己发挥。最后强调一点，本章节所有的`Infix`实现代码都是Zero已经提供的代码，使用时不需额外开发。
